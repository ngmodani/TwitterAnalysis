...
parser.add_argument("count",help = "enter the top no. of time zones you want ",type=int) 
...
for f in file_data:
    if file_data[f]["lang"] =="en":
        ...
        if(tweet_date <= date_end or tweet_date >= date_start):
            time_zone = file_data[f]["user"]["time_zone"]
            if time_zone:
                if time_zone not in popularity:
                    popularity[time_zone] = 1
                else:
                    popularity[time_zone] = popularity[time_zone] +1
